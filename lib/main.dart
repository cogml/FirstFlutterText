...
class MyApp extends StatelessWidget { //MyApp커스텀 위젯 생성
  const MyApp({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return MaterialApp(//title데이터 위젯
      debugShowCheckedModeBanner: false, //오른쪽 상단의 debug띠 없애기
      title: "Hello",
      theme: ThemeData( //theme데이터 위젯
...
class Grade extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: new AppBar( //이러한 위젯들도 클래스로 생성된 것이므로 new를 쓰거나 안쓰거나 할 수 있다
        title: Text('Hello'),
        centerTitle: true, //appbar의 title 가운데 정렬
      ...
      drawer: Drawer(
        child: ListView(
          ...
          children: <Widget>[
            UserAccountsDrawerHeader( //사용자 프로필
              currentAccountPicture: CircleAvatar(
                backgroundImage: AssetImage('assets/img_profile.png'),
                backgroundColor: Colors.amber[50],
              ),
              otherAccountsPictures: <Widget>[ //다른 계정들 표시
                CircleAvatar(
                  backgroundImage: AssetImage('assets/img_profile.png'),
          ...
          children: <Widget>[
            ...
            Center(
              child: Align(
                ...
                child: Image.asset('assets/img_korea.gif',
                height: 50.0,),
              ),
            ),
          ],
